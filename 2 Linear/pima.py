...
pima = numpy.loadtxt('/Users/srmarsla/Book/Datasets/pima/pima-indians-diabetes.data', delimiter=',')
...
p.pcn_train(pima[:, :8], pima[:, 8:9], 0.25, 100)
p.confmat(pima[:, :8], pima[:, 8:9])

# Various preprocessing steps
pima[numpy.where(pima[:, 0] > 8), 0] = 8

pima[numpy.where(pima[:, 7] <= 30), 7] = 1
pima[numpy.where((pima[:, 7] > 30) & (pima[:, 7] <= 40)), 7] = 2
pima[numpy.where((pima[:, 7] > 40) & (pima[:, 7] <= 50)), 7] = 3
pima[numpy.where((pima[:, 7] > 50) & (pima[:, 7] <= 60)), 7] = 4
pima[numpy.where(pima[:, 7] > 60)] = 5
...
p1.pcn_train(trainin, traintgt, 0.25, 100)
